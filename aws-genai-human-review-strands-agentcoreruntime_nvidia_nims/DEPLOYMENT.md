...
The AI agents use [NVIDIA NIMs](https://build.nvidia.com) via an OpenAI-compatible API for inference. The orchestrator uses `gpt-oss-120b` for tool calling, and the sub-agents use `gpt-oss-120b` for content generation.
...
echo "OPENAI_BASE_URL=https://integrate.api.nvidia.com/v1" >> .env
echo "OPENAI_API_KEY=nvapi-your-key-here" >> .env
```
