...
openai                  # OpenAI-compatible client for NVIDIA NIMs
```
...
| **Amazon Bedrock** | AgentCore runtime hosting | Pay per invocation |
| **NVIDIA NIMs** | AI model inference (gpt-oss-120b) | Pay per token |
...
#### Change AI Model
The system uses NVIDIA NIMs (`gpt-oss-120b`) via the OpenAI-compatible API. Configure in `.env`:
```bash
...
| **Bedrock Access Denied** | Ensure AgentCore runtime IAM role has proper permissions |
