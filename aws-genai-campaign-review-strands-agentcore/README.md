...
| **AI-Powered Analysis** | NVIDIA NIMs (gpt-oss-120b) via OpenAI-compatible API |
...
| **Amazon Bedrock** | AgentCore runtime hosting for agent execution |
| **NVIDIA NIMs** | gpt-oss-120b model for AI inference via OpenAI-compatible API |
...
3. **NVIDIA NIMs API Key**:
   - Sign up at [NVIDIA Build](https://build.nvidia.com)
   - Generate an API key for `gpt-oss-120b` model
4. **Service Quotas** (defaults are sufficient):
...
### Step 3b: Configure NVIDIA NIMs API Key
...
#### Change AI Model
Edit `agent.py` and set environment variables:
```bash
# In .env
OPENAI_BASE_URL=https://integrate.api.nvidia.com/v1  # Change endpoint as needed
OPENAI_API_KEY=nvapi-your-key-here
```
The model ID is configured in `agent.py`:
```python
model_id="openai/gpt-oss-120b"  # Change as needed
```
...
| **NVIDIA NIMs Auth Error** | Verify OPENAI_API_KEY is set correctly in .env |
...
**Total estimated cost**: $5-30/month (primarily Bedrock and NVIDIA NIMs usage)
