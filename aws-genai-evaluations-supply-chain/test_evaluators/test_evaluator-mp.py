...
Usage:
    ...
    python test_evaluator.py --api-url <evaluators-api-url> create \
        --metrics "mp_constraint_satisfaction_metric.json,mp_sql_grounding_metric.json,mp_decision_rationale_metric.json"
    ...
    python test_evaluator.py --api-url <evaluators-api-url> run \
        ...
        --evaluators "sc_optimization_constraint-abc123,Builtin.Correctness,Builtin.GoalSuccessRate"
...
def create_evaluators(api_url: str, metrics: list) -> dict:
    """Create custom evaluators from a list of metric JSON filenames.

    Args:
        api_url: Evaluators API Gateway base URL.
        metrics: List of metric JSON filenames to create evaluators from.
    """
    print("\n--- Creating Evaluators ---\n")
...
def create_all_evaluators(api_url: str, metrics_folder: str = "", level: str = "TRACE", prefix_filter: str = "") -> dict:
    """Create evaluators from ALL metric JSON files in a folder.

    Args:
        api_url: Evaluators API Gateway base URL.
        metrics_folder: Optional subfolder within metrics/ to scan.
        level: Evaluation level (TRACE or SESSION).
        prefix_filter: Only process files starting with this prefix.
    """
...
def run_evaluations(api_url: str, agent_id: str, session_id: str, evaluator_ids: list) -> dict:
    """Run evaluations against agent traces with a list of evaluator IDs."""
    print("\n--- Running Evaluations ---\n")
...
def run_builtin_evaluations(api_url: str, agent_id: str, session_id: str) -> dict:
    """Run built-in evaluators (Correctness + GoalSuccessRate) against a session."""
...
def main():
    ...
    create_parser.add_argument("--metrics", required=True, help="Comma-separated list of metric JSON filenames (e.g. 'mp_constraint_satisfaction_metric.json,mp_sql_grounding_metric.json')")

    # Create-all command
    create_all_parser = subparsers.add_parser("create-all", help="Create evaluators from ALL metric JSON files in a folder")
    create_all_parser.add_argument("--metrics-folder", default="", help="Subfolder within metrics/ to scan (default: root metrics/ dir)")
    create_all_parser.add_argument("--level", default="TRACE", help="Evaluation level: TRACE or SESSION (default: TRACE)")
    create_all_parser.add_argument("--prefix-filter", default="", help="Only process files starting with this prefix (e.g. 'mp_')")

    # Run command
    run_parser = subparsers.add_parser("run", help="Run evaluations against a session")
    run_parser.add_argument("--agent-id", required=True, help="Agent runtime name (e.g. supply_chain_orchestrator_agent-YK657xBNWi)")
    run_parser.add_argument("--session-id", required=True, help="Session ID to evaluate")
    run_parser.add_argument("--evaluators", required=True, help="Comma-separated list of evaluator IDs (custom or built-in, at least 1 required)")

    # Run-builtin command
    run_builtin_parser = subparsers.add_parser("run-builtin", help="Run built-in evaluators (Correctness + GoalSuccessRate)")
    ...
    # List-metric command
    list_metric_parser = subparsers.add_parser("list-metric", help="List evaluator ID for a specific metric name")
    list_metric_parser.add_argument("--name", required=True, help="Evaluator name to search for (e.g. 'mp_constraint_satisfaction')")
