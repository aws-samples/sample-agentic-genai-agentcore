...
Pass a comma-separated list of metric JSON filenames from the Lambda's `metrics/` directory:
...
Or create all evaluators from the metrics directory at once:

```bash
# Create ALL evaluators in the metrics/ directory
python test_evaluator.py --api-url "https://<evaluators-api-url>" create-all

# Create only evaluators matching a prefix
python test_evaluator.py --api-url "https://<evaluators-api-url>" create-all \
    --prefix-filter "sc_" --level TRACE
```

The API returns 202 immediately. Evaluator creation runs asynchronously — results are saved to S3 at:
```
s3://<agent-source-bucket>/evaluators/creation/<timestamp>/results.json
```

#### Step 1b: List Evaluators

```bash
# List all evaluators
python test_evaluator.py --api-url "https://<evaluators-api-url>" list-all

# List a specific evaluator by name
python test_evaluator.py --api-url "https://<evaluators-api-url>" list-metric \
    --name "sc_optimization_constraint"
```
...
#### Step 3: Run Built-in Evaluators Only

A convenience command that runs `Builtin.Correctness` and `Builtin.GoalSuccessRate` without needing to specify evaluator IDs:

```bash
python test_evaluator.py --api-url "https://<evaluators-api-url>" run-builtin \
    --agent-id "supply_chain_orchestrator_agent-YK657xBNWi" \
    --session-id "<session-id>"
```
...
#### Step 4: Delete Evaluators (cleanup)

Deletion is also async (returns 202):
...
|---------|--------|-------------|
| `create` | POST /evaluators/create | Create evaluators from specific metric JSON files |
| `create-all` | POST /evaluators/create-all | Create evaluators from all metrics in the directory |
| `run` | POST /evaluators/run | Run evaluations with custom and/or built-in evaluator IDs |
| `run-builtin` | POST /evaluators/run-builtin | Run Correctness + GoalSuccessRate |
| `list-all` | GET /evaluators/list | List all registered evaluators |
| `list-metric` | GET /evaluators/list?name=X | Find evaluator by name |
| `delete` | DELETE /evaluators/delete | Delete evaluators by ID |

All mutating operations (create, run, delete) are asynchronous — the API returns 202 immediately and the Lambda processes the work via self-invocation.
